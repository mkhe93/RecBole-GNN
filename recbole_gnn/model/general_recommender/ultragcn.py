...
class UltraGCN(GeneralGraphRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        print(f"New operation completed in {elapsed_time:.2f} seconds.")

        #start_time = time.time()
        #self.ii_neighbor_mat_old, self.ii_constraint_mat_old = self.get_ii_constraint_mat_old(dataset.inter_matrix(form="csr"), self.ii_neighbor_num)
        #end_time = time.time()
        # Calculate duration
        #elapsed_time = end_time - start_time
        #print(f"Old operation completed in {elapsed_time:.2f} seconds.")
    ...
    def predict_old(self, interaction):
    ...
    def get_ii_constraint_mat_old(self, train_mat: csr_matrix, num_neighbors: int, ii_diagonal_zero=False):
        print('Computing \\Omega for the item-item graph...')

        # Compute item-item similarity matrix (A)
        ...
        # Set diagonal to zero if required
        if ii_diagonal_zero:
            A.setdiag(0)
            A.eliminate_zeros()  # Ensure sparsity is maintained

        # Compute degree vectors
        items_D = np.array(A.sum(axis=0)).reshape(-1)  # Column-wise sum (item degree)
        users_D = np.array(A.sum(axis=1)).reshape(-1)  # Row-wise sum (item degree)
        ...
        for i in range(n_items):
            # Multiply the constraint matrix with the similarity row
            row = all_ii_constraint_mat[i] * torch.from_numpy(A.getrow(i).toarray()[0])

            # Find top-k neighbors (indices and similarities)
            ...
            if i % 15000 == 0:
                print(f'i-i constraint matrix {i} ok')
